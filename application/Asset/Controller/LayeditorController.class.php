...
class LayeditorController extends Controller {
	...
	public function uploadimg(){
		...
		if ($info) {
			...
			//上传成功
			$title = $oriName = $info['file']['name'];
			
			$state = 'SUCCESS';
			$code=0;
			$file = C("TMPL_PARSE_STRING.__UPLOAD__")."ueditor/".$info['file']['savename'];
		...
		$response=json_encode(array("code"=>$code,"msg"=>$state,"data"=>array("src"=>$file,"title"=>$title)));
		exit($response);
	}
