	...
	function _initialize() {
		...
		if($ucenter_syn){
			if(!isset($_SESSION["user"])){
				if(!empty($_COOKIE['htcms_auth'])  && $_COOKIE['htcms_auth']!="logout"){
					$htcms_auth=sp_authcode($_COOKIE['htcms_auth'],"DECODE");
					$htcms_auth=explode("\t", $htcms_auth);
					$auth_username=$htcms_auth[1];
					$users_model=M('Users');
